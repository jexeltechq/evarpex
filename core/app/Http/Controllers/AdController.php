...
class AdController extends Controller
{
    ...
    public function storeAd(Request $request)
    {
           ...
           $ad->status = 'pending'; // or $ad->is_available = false;
           $ad->save();
           if($request->image){
    ...
    public function initiateEscrow($adId)
    {
        $ad = AdList::findOrFail($adId);

        // Prevent buying your own ad
        if ($ad->user_id == Auth::id()) {
            $notify[] = ['error', 'You cannot buy your own ad.'];
            return back()->withNotify($notify);
        }

        // Check if an escrow already exists for this ad and buyer
        $existing = Escrow::where('ad_id', $ad->id)
            ->where('buyer_id', Auth::id())
            ->whereIn('status', ['pending', 'funded'])
            ->first();

        if ($existing) {
            $notify[] = ['error', 'You already have a pending/funded escrow for this ad.'];
            return back()->withNotify($notify);
        }
        ...
        return redirect()->route('escrow.pay', $escrow->id);
    }

    public function buyWithEscrow($id)
    {
        $ad = AdList::findOrFail($id);

        // Prevent seller from buying their own ad
        if ($ad->user_id == Auth::id()) {
            return back()->with('error', 'You cannot buy your own ad.');
        }

        // Mark ad as unavailable
        $ad->status = 'pending'; // or 'in_escrow'
        $ad->save();
        ...
        // Redirect to payment page (replace with your Paystack logic)
        return redirect()->route('paystack.pay', ['escrow' => $escrow->id]);
    }
}
