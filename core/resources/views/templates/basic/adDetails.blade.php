        ...
        <div class="row justify-content-between">
          ...
          <div class="col-lg-4 col-xxl-3 mt-lg-0 mt-5">
            <div class="ad-details-sidebar">
              ...
              @auth
              @if(auth()->id() !== $ad->user_id)
                <form action="{{ route('ad.buy.escrow', $ad->id) }}" method="POST" class="mb-3">
                    @csrf
                    <button type="submit" class="btn btn-md btn--success w-100">
                        <i class="las la-shopping-cart"></i> Buy Now (Escrow)
                    </button>
                </form>
                @endif
              @endauth
...
@auth
    @if(auth()->id() === $escrow->buyer_id && $escrow->status === 'funded')
        <form action="{{ route('escrow.release', $escrow->id) }}" method="POST">
            @csrf
